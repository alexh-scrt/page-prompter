...
class TestBuildPlainText:
    ...
    def test_contains_annotation_count(self) -> None:
        annotations = [make_annotation(), make_annotation(selector=".nav", annotation_id="uuid-002")]
    ...
    def test_annotation_id_not_present_when_none(self) -> None:
        ...
        result = _build_plain_text(PAGE_URL, [ann])
        # Should not raise or contain 'None' for an id label
    ...
    def test_empty_html_context_not_shown_as_section(self) -> None:
        ...
        result = _build_plain_text(PAGE_URL, [ann])
        # Should not produce 'HTML context:' header when context is empty
...
class TestBuildXmlPrompt:
    ...
    def test_contains_annotation_count_nonzero(self) -> None:
        anns = [make_annotation(), make_annotation(selector=".nav", annotation_id="id-2")]
    ...
    def test_single_annotation_html_context_present(self) -> None:
        ...
        result = _build_xml_prompt(PAGE_URL, [ann])
        # Context is XML-escaped inside the tag
    ...
    def test_special_chars_in_url_are_escaped(self) -> None:
        ...
        assert "&amp;" in result
        # Raw ampersand should not appear inside an XML element
        # (it's OK in the XML declaration line but not in tag content)
        lines_with_url = [l for l in result.splitlines() if "example.com" in l]
...
class TestBuildPromptExport:
    ...
    def test_multiple_annotations_count(self) -> None:
        anns = [make_annotation(selector=f"#{i}", annotation_id=f"id-{i}") for i in range(5)]
    ...
    def test_all_formats_contain_page_url(self) -> None:
        ...
        # URL may be XML-escaped in xml_prompt if it contains special chars;
        # for a clean URL it should appear literally
...
class TestEdgeCases:
    ...
    def test_annotation_with_multiline_html_context(self) -> None:
        ...
        result = build_prompt_export(PAGE_URL, [ann])
        # Multi-line context should be preserved in some form in all formats
    ...
    def test_comment_with_xml_special_chars(self) -> None:
        ...
        result = build_prompt_export(PAGE_URL, [ann])
        # Plain text may contain raw chars
        assert "<h1>" in result.plain_text
        # XML prompt must escape them
